...
<div class="app-container">
    <div v-if="type != 'business'">
        ...
        <el-alert title="重置的登录密码和支付密码：admin123456" :closable="false" style="margin-bottom: 12px;" type="warning">
        </el-alert>
        <el-table :key="tableKey" v-loading="listLoading" :data="list" border fit ref="table" highlight-current-row @expand-change="searchBalances">
    ...
    <div v-else>
        <el-table :key="1" v-loading="listLoading" :data="rateList" border fit highlight-current-row>
            <!-- @sort-change="sortChange" -->
    ...
    <el-dialog :close-on-click-modal='false' :close-on-press-escape='false' :title="dialogStatus1" :visible.sync="dialogFormVisible1">
        <el-form ref="dataForm1" :model="temp1" label-position="left" label-width="140px" style="width: 400px; margin-left:36px;">
            <div v-for="item,index in merchantRate" :key="index">
                <el-alert v-if="item.key == 'mixFee'" title="以下字段-1代表不限制" :closable="false" style="margin-bottom: 12px;" type="warning">
                </el-alert>
    ...
    <el-dialog :close-on-click-modal='false' :close-on-press-escape='false' title="安全校验" :visible.sync="ipVerifyDialog" center>
        <el-form ref="dataForm2" :model="temp2" :rules="rules2" label-position="left" label-width="120px" style="width: 400px; margin-left:36px;">
            <el-form-item label="开启/关闭" prop="ipVerify">
                <el-select v-model="temp2.ipVerify" filterable placeholder="请选择">
                    <el-option v-for="item in ipVerifyList" :key="item.value" :label="item.name" :value="item.value">
                    </el-option>
                </el-select>
            </el-form-item>
            <el-form-item :label="'商户是否测试'" prop="isTest">
                <el-select v-model="temp2.isTest" filterable placeholder="请选择">
                    <el-option v-for="item in [{name:'是',value: 'Y'},{name:'否',value: 'N'}]" :key="item.value" :label="item.name" :value="item.value">
                    </el-option>
                </el-select>
            </el-form-item>
    ...
    <el-dialog :close-on-click-modal='false' :close-on-press-escape='false' :title="`解冻审核`" :visible.sync="dialogClearGooleAuth" center>
...
export default {
    ...
    async created() {
        for (let i of merchant) {
            ...
            if (i.type == 'select' && i.key != 'isTest') {
                i.list = (await utilsApi.dictionaryFindPage({
                    type: i.listKey || i.key
                })).data.records
            }
        }
        this.getList()
    ...
    methods: {
        ...
        async handleDelete(row, index) {
            this.$alert('确定删除该数据？', '删除', {
                ...
                callback: async action => {
                    ...
                    let res = await utilsApi.delDictionary({
                        id: row.id
                    })
                    if (res.code == 0) {
                        this.$notify({
        ...
        resetLogPassword(row) {
            this.$alert(`确定重置该商户的登录密码吗？`, '提示', {
                ...
                callback: async action => {
                    ...
                    let res = await utilsApi.resetLogPassword({
        ...
        resetPayPassword(row) {
            this.$alert(`确定重置该商户的支付密码吗？`, '提示', {
                ...
                callback: async action => {
                    ...
                    let res = await utilsApi.resetPayPassword({
                        merchantCode: row.merchantCode
                    })
                    if (res.code == 0) {
                        this.$notify({
        ...
        async handleBusinessStatus(row, status) {
            this.$alert(`确定${status == 'Y' ? '启用' : '禁用'}该商户业务？`, '提示', {
                ...
                callback: async action => {
                    ...
                    let res = await utilsApi.updateBusinessStatus({
                        merchantBusinessId: row.id,
                        status
                    })
                    if (res.code == 0) {
                        this.$notify({
        ...
        async handleStatus(row, status) {
            this.$alert(`确定${status == 'Y' ? '启用' : '禁用'}该商户？`, '提示', {
                ...
                callback: async action => {
                    ...
                    let res = await utilsApi.updateMerchantStatus({
                        merchantCode: row.merchantCode,
                        status
                    })
                    if (res.code == 0) {
        ...
        async findMerchantRate(code) {
            this.listLoading = true
            let res = await utilsApi.findMerchantRate({
                merchantCode: this.currentRow.merchantCode
            })
            this.listLoading = false
        ...
        async sendCode() {
            ...
            let res = await utilsApi.sendCode({
                type: '清除商户谷歌验证器'
            })
        ...
        clearGooleAuthConfirm() {
            this.$refs['clearGooleAuthForm'].validate(async (valid) => {
                if (valid) {
                    this.buttonLoading = true
                    let res = await utilsApi.clearMercahntGoolAuth({
                        id: this.currentRow.id,
                        verifCode: this.temp3.verifCode,
                        emailCode: this.temp3.emailCode
                    })
                    this.buttonLoading = false
